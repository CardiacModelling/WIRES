...
# Score
problem = pints.SingleOutputProblem(stair_herg, times, data)
...
# Store output
with open('%s/stair-fit-%s.txt' % (savedir, fit_id), 'w') as f:
    for k, x in enumerate(p):
...
axes[0, 0].plot(times, stair_herg.v_func(times), c='#7f7f7f')
...
predict = ap_herg.simulate(p, times)
axes[0, 1].plot(times, ap_herg.v_func(times), c='#7f7f7f')
...
plt.savefig('%s/stair-fit-%s.png' % (savedir, fit_id), dpi=200,
        bbox_inches='tight')
plt.close()
